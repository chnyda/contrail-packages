...
source-package-contrail: clean-contrail debian-contrail
	...
	# remove INSTALL_SERIES from .install files
	(cd build/packages/$(PACKAGE)/debian; sed -i '/INSTALL_SERIES/d' *.install)
	...
	# remove DIRS_SERIES from .dirs files
	(cd build/packages/$(PACKAGE)/debian; sed -i '/DIRS_SERIES/d' *.dirs)
...
source-package-ifmap-server: clean-ifmap-server debian-ifmap-server source-ifmap-server
	$(eval PACKAGE := ifmap-server)
	(cd build/packages/$(PACKAGE); sed -i 's/SERIES/$(SERIES)/g' debian/changelog)
	(cd build/packages/$(PACKAGE); dpkg-buildpackage -sa -S -d -rfakeroot $(KEYOPT))
